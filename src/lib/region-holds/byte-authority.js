...
class ByteAuthority {
  constructor({ stateRoot, materialize, publish, prepareCreate, reconcileCreateStage, now = () => Date.now(), lockTimeoutMs = LOCK_TIMEOUT_MS } = {}) {
    ...
    const directory = stateRoot
      ? path.join(assertAccountProfilePath(stateRoot, { field: 'byte coordination state root', requireOwnedProfile: true }), 'state', 'byte-coordination')
      : statePath('state', 'byte-coordination');
  ...
  async _snapshot(resource, materialize = this.materialize) {
    ...
    if (!value || (value.present !== undefined && value.present !== true)
        || !Buffer.isBuffer(value.bytes) || value.bytes.length > MAX_RESOURCE_BYTES) {
  ...
  async observeRead({ binding: rawBinding, resource: rawResource, startByte, endByte, materializeRead, validateRead, assertCurrent } = {}) {
    ...
    return this._locked(async db => {
      ...
      const invalidations = db.prepare('SELECT resource,receipt_ref,stale_reason FROM reads WHERE scope_id=? AND stale_reason IS NOT NULL')
  ...
  async _validateReadSet(db, binding, snapshots) {
    const initial = db.prepare('SELECT DISTINCT resource FROM reads WHERE scope_id=?').all(binding.runtimeScopeId);
    const unavailable = [];
    ...
    const at = this._time();
    for (const row of db.prepare('SELECT * FROM reads WHERE scope_id=?').all(binding.runtimeScopeId)) {
  ...
  _commitOperation(db, operation, { recovered = false } = {}) {
  ...
  async applyPatch({ binding: rawBinding, resource: rawResource, receiptRef, derivePatch, assertCurrent } = {}) {
    ...
    return this._locked(async db => {
      ...
      await this._validateReadSet(db, binding, new Map([[resource, snapshot]]));
      ...
      if (after.length > MAX_RESOURCE_BYTES) refuse('BYTE_PATCH_INVALID', 'The resulting file exceeds the repository file bound.');
  ...
  async applyWrite({ binding: rawBinding, resource: rawResource, bytes, assertCurrent } = {}) {
    const binding = normalizeBinding(rawBinding);
    const resource = resourceKey(rawResource);
    if (!Buffer.isBuffer(bytes) || bytes.length > MAX_RESOURCE_BYTES) {
    ...
    return this._locked(async db => {
      ...
      await this._validateReadSet(db, binding, new Map([[resource, before]]));
      ...
      return { receipt, bytes: receipt.bytes, created: !before.present };
    });
  }
  ...
  async _publishOperation(db, operation, before, after, assertCurrent) {
    ...
    this._transaction(db, () => {
      ...
      this._event(db, operation.op + '.prepared', operation);
    });
    try {
      ...
      const receipt = this._commitOperation(db, operation);
  ...
  _validateOperation(db, row, operation) {
    ...
    const invalid = operation.operationId !== row.id || !OPERATION_ID.test(operation.operationId)
      ...
      || !Number.isSafeInteger(operation.beforeBytes) || operation.beforeBytes < 0 || operation.beforeBytes > MAX_RESOURCE_BYTES
      || !Number.isSafeInteger(operation.afterBytes) || operation.afterBytes < 0 || operation.afterBytes > MAX_RESOURCE_BYTES
