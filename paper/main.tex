...
\item[I2 Admission-time validation.] An admitted patch of $[s,e)$ by $\sigma$ passes a read-set check in which every observation is valid at validation time $t_{\mathrm v}$, followed by a coverage check on the valid observations of one receipt of $\sigma$ on $r$. Expiry is checked during admission; it is not rechecked as a publication deadline.
...
\item[I5 Journal.] PREPARED is durable before publication. COMMITTED implies the after-image, and for a creation the staged identity, was confirmed on disk. Recovery resolves a non-no-op PREPARED operation to ABORTED on the before-image, to COMMITTED on the after-image, or to UNKNOWN otherwise. A pending no-op commits without a new publication. An operation's read-set effects and version increment are applied exactly when it becomes COMMITTED.
\end{description}
...
Sketch. Let patch $P$ by $\sigma$ replace $[s,e)$. By I2, admission establishes valid observations covering $[s,e)$, so $F_r[s{:}e]$ equals what $\sigma$ observed at that check. The same authority lock excludes another mediated writer through publication; lease expiry alone does not change the bytes. Suppose a write $W$ by another scope changed a byte in $[s,e)$ after $\sigma$ observed it. By I3, $W$'s commit marked $\sigma$'s conflicting observation stale, which contradicts validity. So $\sigma$ must have reread after $W$, and the byte's latest write is one it observed. Bytes outside $[s,e)$ are untouched, and their observations are shifted, not invalidated. An unmediated change before $P$'s admission is caught by I4, because admission materializes $r$. A change between the publisher's final comparison and its rename is outside the proposition's assumption.

The same argument, applied to observations on other files, gives read-set freshness. Provided every writer of every observed resource uses the same authority, the lock preserves the validated byte contents through publication. This covers an intervening mediated interface change, but it does not fence an unmediated writer that changes a dependency after that dependency was checked. A receipt's lease can also expire after admission and before publication; the implementation has no final read-set expiry check. These are sketches of the mechanism, not machine-checked proofs.
...
\paragraph{Admission-boundary review.}
Two additional controlled checks reproduce the boundaries of I2. Advancing the injected clock past the read lease after admission still permits publication; changing an observed dependency through an unmediated writer after read-set validation also permits publication on the target. These checks document limits, not desired application behavior, and are separate from the 68 core tests and six original probes. Source and captured output are in \code{review/}. No engine code was changed by this review.
...
In the inspected ToolsEnabled engine snapshot (source commit \code{c34cab8c}; engine package metadata 1.4.0), the authority was wired to the \code{repo.*} tools and the paired-desktop read path. The \code{repo.*} tools resolve paths against the engine's own package root. In an installed build that root is the installed payload, and generic writes into it are refused. The agents in that deployment edited their working clones through \code{host.read\_file}, \code{host.write\_file} and \code{host.exec}. The host file provider takes a whole-file write lock and records an audit entry, but never consults the authority. The authority was therefore bypassed by that coding workflow. No production conflict dataset accompanies this reviewed workflow, and none is reported here. These source and per-suite receipts are not an installed-release acceptance receipt for the final 1.0.44 cut. A mediation layer protects only the write paths routed through it, and the paths these agents used were not.

Mediation of \code{host.read\_file}, \code{host.write\_file} and a new \code{host.patch\_file} through the same authority has since been implemented on an engine branch (tip \code{3c40a8d4}). It is ready for repackaging but not yet deployed. It uses a store namespace separate from \code{repo.*}, prunes committed payloads at commit, and adds an off switch. Unlike \code{repo.write\_file}, \code{host.write\_file} replaces an existing file only when the writing scope currently observes all of it, so the P3 scenario refuses for host files. Its new 11-test suite covers these rules, retention, crash recovery and an end-to-end call through the engine's MCP dispatch. All 11 fail at the base engine, the first because agent A's stale write silently overwrites B's; all 11 pass on the branch (\code{data/host-mediation-branch/}).

The branch also reports two measurements, taken on a machine that was running other work. First, over 100 interleaved iterations on a 100\,KB file, a \code{host.read\_file} plus \code{host.write\_file} round trip through the engine's tool dispatcher took 266.01\,ms at the median and 334.26\,ms at the 95th percentile with mediation, against 213.73\,ms and 266.01\,ms without it. Second, over 500 writes of 100\,KB, the host store grew by 141,148,160 bytes when committed payloads were kept and by 4,931,584 bytes when they were pruned at commit.
...
\item \textbf{Coverage.} Unmediated writers, including shell commands, are detected only when next observed, and a change landing between the final comparison and the rename is not caught. Host file mediation (Section~\ref{sec:deploy}) is not yet deployed.
...
\item \textbf{Retention.} The extracted core retains committed payloads (P7). The separately reported host-mediation branch adds pruning; that does not change the behavior of this extraction.
