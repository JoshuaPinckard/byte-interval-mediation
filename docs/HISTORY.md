...
Host-file mediation is now implemented on an engine branch (tip `3c40a8d4`), ready for repackaging and not yet deployed. It routes `host.read_file`, `host.write_file` and a new `host.patch_file` through the same authority, in a store namespace separate from `repo.*`. Committed payloads are pruned at commit, and there is an off switch. `host.write_file` replaces an existing file only when the writing scope currently observes all of it.

Its new 11-test suite fails 11 of 11 at the base engine and passes 11 of 11 on the branch. The branch also reports two measurements. A 100 KB read-and-write round trip takes 266.01 ms at the median with mediation, against 213.73 ms without. Over 500 writes of 100 KB, the store grows 141,148,160 bytes with payloads kept, against 4,931,584 bytes with them pruned. The logs and measurements are in `data/host-mediation-branch/`.
